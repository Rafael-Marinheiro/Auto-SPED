# -*- coding: utf-8 -*-
"""Utilitários para geração do arquivo SPED."""
...
@dataclass
class RegisterEntry:
    """Representa um registro individual informado pelo usuário."""
    ...
    def create(cls, code: str, data: Dict[str, str]) -> "RegisterEntry":
        ...
        if definition.block == "9":
            raise ValueError(
                "Registros do bloco 9 são gerados automaticamente e não devem ser informados."
            )
        values = definition.ordered_values(data)
        # ordered_values já valida a presença dos campos obrigatórios.
...
class SpedConfig:
    ...
    def from_dict(cls, payload: Dict) -> "SpedConfig":
        if "registers" not in payload or not isinstance(payload["registers"], list):
            raise ValueError("O arquivo de configuração deve possuir a chave 'registers'.")
...
class SpedWriter:
    """Responsável por montar o arquivo SPED a partir dos registros fornecidos."""
    ...
    def validate(self) -> None:
        if not self._entries:
            raise ValueError("É necessário informar ao menos o registro 0000.")
        ...
        if "0001" not in codes:
            raise ValueError("O registro 0001 é obrigatório no bloco 0.")
    ...
    def to_lines(self) -> List[str]:
        ...
        if not block_0_entries:
            raise ValueError("O bloco 0 é obrigatório e não foi informado.")
        ...
        # Bloco E - versão mínima (E001/E990)
    ...
    def generate_sped_from_db(self, company_info, accountant_info, participants, products, units, invoices, extractor, start_date, end_date, log_fn: Callable[[str], None] | None = None, revenue_code: str | None = None, layout_version: str | None = None):
        ...
        for participant in participants:
            self.add_register("0150", {
                ...
                "COD_PAIS": "1058",  # Brasil (código IBGE)
        ...
        for idx, invoice in enumerate(invoices, start=1):
            ...
            # Define IND_EMIT conforme leiaute: 0=Própria, 1=Terceiros
            if cod_mod == "65":
                invoice["IND_OPER"] = "1"
                invoice["IND_EMIT"] = "0"
            elif cod_mod == "55":
        ...
        def _to_decimal(x) -> Decimal:
            ...
            if "," in s and "." in s:
                # supõe ponto como milhar e vírgula como decimal
                s = s.replace(".", "").replace(",", ".")
            else:
                s = s.replace(",", ".")
            try:
        ...
        for inv in invoices:
            ...
            # Saneamento: ignora valores de ICMS fora de faixa razoável
            if vl_doc <= 0:
                continue
            if vl_icms < 0 or vl_icms > vl_doc:
                continue
            # NFC-e sempre saída (débito)
            if cod_mod_inv == "65":
        ...
        # E110 - Apuração consolidada (mínima)
